...
def multiview_plotting(data,labels,control_point, savepath,classes=5,alpha=0.5,nbviews=3,plot_labels=True):
...
"""
    ...
    for i in range(nbviews):
        if i == 0 :
            ...
            if plot_labels:
                ...
                ax.imshow(data[control_point[0], :, :], cmap=cm.gray)
                ax.imshow(labels[control_point[0], :, :], cmap=my_cm,alpha=alpha)
                ax.scatter(y=control_point[1], x=control_point[2], c='r', s=2)
                ax.set_xticklabels([])
                ax.set_yticklabels([])
            ...
            if plot_labels:
                ax = plt.subplot(outer_grid[i+nbviews])
                ax.imshow(data[:, control_point[1], :], cmap=cm.gray,aspect=(data.shape[1]/data.shape[0]))
                ax.imshow(labels[:, control_point[1], :], cmap=my_cm,alpha=alpha,aspect=(data.shape[1]/data.shape[0]))
                ax.scatter(y=control_point[0], x=control_point[2], c='r', s=2)
                ax.set_xticklabels([])
                ax.set_yticklabels([])
            ...
            if  plot_labels:
                ...
                ax = plt.subplot(outer_grid[i + nbviews])
                ax.imshow(img, cmap=cm.gray, aspect=(data.shape[1] / data.shape[0]))
                ax.imshow(img_label, cmap=my_cm,alpha=alpha, aspect=(data.shape[1] / data.shape[0]))
                ax.scatter(y=control_point[0], x=control_point[2], c='r', s=2)
                ax.set_xticklabels([])
                ax.set_yticklabels([])
    ...
    plt.savefig(savepath, transparent=True,bbbox_inches='tight',pad_inches=0,dpi=600)
    plt.close(fig)
